...
int itemCompare(ItemType item1, char title[]) {
    if (item1.Title == title) /*** esempio ***/
        return 0;
    else
        return -1;
}
...
ItemType* Find(LIST l, char title[]) {
    LIST tmp = l;
    /*if (tmp == NULL) {
      return NULL;
    }*/
    while (tmp != NULL && itemCompare(tmp->item, title) != 0) {
        tmp = tmp->next;
    }
    if (tmp == NULL) {
        return NULL;
...
LIST Dequeue(LIST l, char title[]) {
    if (!isEmpty(l)) {
        if (itemCompare(l->item, title) == 0) {
            ...
            while (!isEmpty(tmp->next) &&
                   itemCompare(tmp->next->item, title) != 0)
                tmp = tmp->next;
